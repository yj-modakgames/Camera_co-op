...
> 작성 2026-08-26 · 상태: **구현·검증 완료** (branch phase2-drawing. DoD D-1~D-7 전 항목 통과 — D-4는 재검출 스냅 가드(§6) 추가 후 통과. 품질 채점 9.9/10)
> 알려진 한계: 화면 가장자리에서 MediaPipe handedness 라벨이 간헐적으로 흔들려 반대색 티끌 스트로크(점 2개 미만, 대부분 자동 폐기)가 드물게 생길 수 있다. 실사용 영향 미미로 판단, Phase 3에서 관찰.
