...
namespace CameraCoop
{
    ...
    public class HandInputRouter : MonoBehaviour
    {
        ...
        internal void ProcessSample(HandInputSample sample, float now, HandInteractable target, Vector3 hitPosition)
        {
            ...
            if (HasCapture(hand))
            {
                hand.capturePosition = hitPosition;
                if (sample.isPinched)
                {
                    hand.capture.Hold(sample, hitPosition);
                }
                else if (wasPinched)
                {
                    ReleaseCapture(hand, hitPosition, now);
                }
            }
            else if (canPress && IsAvailable(target) && CanDeliver(target) && !OwnedByOtherHand(hand, target))
            {
                ...
                target.Press(sample, hitPosition, hand.pressContext);
            }
            UpdateFeedback();
        }
        ...
        private bool CanDeliver(HandInteractable target)
        {
            return routingEnabled && isActiveAndEnabled && hasFocus && inputModeManager != null &&
                (target.IsCanvas ? inputModeManager.CanDraw : inputModeManager.CanUseHandUi);
        }
